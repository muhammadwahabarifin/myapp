...
export default function Hero({ id }: AnimatedProps) {
  return (
    <section id={id} className='relative min-h-screen flex items-center justify-center px-4 overflow-hidden'>
      ...
      <div className="relative max-w-4xl mx-auto text-center">
        <div className="relative">
          <motion.h1
            className="text-6xl md:text-7xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-foreground via-foreground/90 to-foreground/80 pb-2 capitalize"
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, ease: "easeOut" }}
          >
            Build your <span className='text-light dark:text-melon'>next project</span> with us
          </motion.h1>
          <motion.div
        ...
        <div className="flex items-center justify-center gap-4 relative">
          <motion.div
            ...
            whileTap={{ scale: 0.95 }}
          >
            <Button className="bg-gradient-to-r from-melon to-melon/80 px-8 py-6 text-lg rounded-full transition-all duration-300 text-black">
              Get Started
            </Button>
          </motion.div>
          <motion.div
          ...
          >
            <Button
              ...
              className="border-foreground/20 text-foreground bg-foreground/10 hover:bg-foreground/10 px-8 py-6 text-lg rounded-full transition-all duration-300"
            >
              <a
